...
type BulkProductFormValues = z.infer<typeof bulkFormSchema>;
 ...
 const BulkProductForm = () => {
  ...
  const onSubmit = async (data: BulkProductFormValues) => {
    try {
      setLoading(true);
      await axios.post(`/api/${params.StoreId}/products/multiadd`, data.products);
      router.refresh();
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between">
        <Heading title="Add Multiple Products" description="Create multiple products at once." />
        <Button
          type="button"
          onClick={() => append({
            name: "",
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          {fields.map((field, index) => (
            <div key={field.id} className="space-y-8 p-4 border rounded-lg relative">
              <Button
                type="button"
              ...
              <Heading title={`Product ${index + 1}`} description={`Enter details for product ${index + 1}`} />
